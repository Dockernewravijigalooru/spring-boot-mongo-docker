...
pipeline {
    ...
    stages {
		...
		sh label: '', script: 'ssh -o StrictHostKeyChecking=no ubuntu@13.233.49.242 docker login -u admin -p ${nexus_PWD } 13.233.122.83:8084'
                ...
                sh label: '', script: 'ssh -o StrictHostKeyChecking=no ubuntu@13.233.49.242 docker service create --name spring -d -p 8080:8080 --with-registry-auth 13.233.122.83:8084/spring_mongoapp:${BUILD_NUMBER}'
